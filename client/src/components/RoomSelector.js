...
function SpaceSelector({ selectedRoomId, resetSelectedRoom, handleRoomClick }) {
    ...
    useEffect(() => {
        ...
        const fetchRooms = async () => {
            ...
            try {
                const response = await fetch(`/api/selection?floor=${floor}`, { timeout: 5000 });
    ...
    useEffect(() => {
        if (roomsRefs.current.length > 0) {
            gsap.fromTo(roomsRefs.current, { opacity: 0, scale:0}, { opacity: 1, scale:1, stagger: 0.1, duration:0.2 });
        }
    }, [rooms]);
    ...
    useEffect(()=>{
        if(loading){
            ...
            gsap.to(load.current, {x:0, opacity:0 ,duration:1});
        }
    })
                    ...
                    {rooms.map((room, index) => (
                        <Grid item xs={3} sm={4} lg={4} key={room.id}>
                            <div className='buttonContainer'>
                                ...
                                className='btn-flip' 
                                data-back={`${room.room_number}`} 
                                data-front={`${room.room_number}`} 
